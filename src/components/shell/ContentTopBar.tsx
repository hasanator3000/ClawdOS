      ...
      <div className="flex items-center gap-2.5 mb-4 md:mb-6">
        ...
        <div className="relative flex-1">
          ...
          <input
            ...
            placeholder="Search..."
            className="w-full pl-10 pr-4 md:pr-14 py-2.5 rounded-xl text-sm outline-none bg-[var(--card)] border border-[var(--border)] text-[var(--fg)] focus:border-[var(--neon)] focus:shadow-[0_0_0_1px_var(--neon-dim),0_0_12px_var(--neon-dim)]"
          />
          {/* ⌘K badge — desktop only */}
          <div className="hidden md:flex absolute right-3 top-1/2 -translate-y-1/2 font-mono text-[10px] px-1.5 py-0.5 rounded text-[var(--muted)] border border-[var(--border)]">
            ⌘K
          </div>
        ...
        {/* Chat toggle — desktop only, mobile uses FAB */}
        {aiPanel.isHydrated && (
          <button
            type="button"
            onClick={aiPanel.toggle}
            className="hidden md:flex flex-shrink-0 items-center justify-center w-[38px] h-[38px] rounded-xl transition-colors"
            style={{
...
/** Mobile-only: hamburger (opens drawer) + brand */
function MobileHeaderRow() {
  return (
    <div className="md:hidden flex items-center justify-between mb-3">
      ...
      <button
        type="button"
        onClick={() => window.dispatchEvent(new Event('clawdos:drawer-open'))}
        className="p-2 -ml-1 rounded-lg text-[var(--muted)] active:bg-[var(--hover)] transition-colors"
